      ...
      <header className="bg-background border-b sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center gap-4">
          <Button variant="ghost" size="icon" asChild>
            <Link href="/help" aria-label={t.common.back}>
              <BackIcon className="w-5 h-5" />
            </Link>
          </Button>
          <div>
            <h1 className="text-xl font-bold">{t.helpPages.shopping.title}</h1>
            <p className="text-sm text-muted-foreground">{t.helpPages.shopping.subtitle}</p>
          </div>
        </div>
      </header>

      <main className={`max-w-4xl mx-auto px-4 py-8 ${animationClass}`}>
        {/* 购物流程 */}
            ...
            {tips.map((tip, index) => (
              <Card key={index}>
                <CardContent className="p-4">
                  <h4 className={`font-medium mb-2 flex items-center gap-2 ${isRTL ? 'flex-row-reverse justify-end' : ''}`}>
                    <CheckCircle className="w-4 h-4 text-green-600" />
                    {tip.title}
                  </h4>
                  <p className={`text-sm text-muted-foreground ${isRTL ? 'text-right' : ''}`}>{tip.content}</p>
                </CardContent>
              </Card>
            ))}
          </div>
        ...
        {/* 底部导航 */}
        <div className={`flex justify-between items-center pt-8 border-t ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <Button variant="outline" asChild>
            <Link href="/help/payment">
              {t.helpPages.payment.title}
              <NextIcon className={`w-4 h-4 ${isRTL ? 'mr-2' : 'ml-2'}`} />
            </Link>
          </Button>
        </div>
      </main>
