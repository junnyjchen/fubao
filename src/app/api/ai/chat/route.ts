...
// 获取相关知识（简单实现，实际应连接数据库）
async function getRelevantKnowledge(query: string): Promise<string | null> {
  try {
    // 如果有知识库，可以在这里实现向量搜索或关键词匹配
    // 这里返回null表示不启用知识库增强
    // 实际部署时可以连接MySQL数据库查询ai_training_knowledge表
    return null;
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 构建消息列表，先获取增强的系统提示词
    const enhancedSystemPrompt = await buildEnhancedSystemPrompt(messages);
